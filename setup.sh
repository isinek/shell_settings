...
function install_module() {
  ...
  if [[ "bash" == "${module}" ]]; then
    if [[ ! grep -qs "[ -f ~/.bashrc.user ] && source ~/.bashrc.user" ~/.bashrc ]]; then
